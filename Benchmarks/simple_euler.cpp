...
// solve velocity Verlet differential equations
  if( atoi(argv[1]) == 2 ){
      while (time <= FinalTime){
          double acc_x = FourPi2*x/(r*r*r);
          double acc_y = FourPi2*y/(r*r*r);

          x += Step*vx + acc_x * Step * Step/2.;
          y += Step*vy + acc_y * Step * Step/2.;

          r = sqrt(x*x+y*y);

          double accx_next = FourPi2*x/(r*r*r);
          double accy_next = FourPi2*y/(r*r*r);

          vx -= accx_next * Step/2. + acc_x * Step/2.;
          vy -= accy_next * Step/2. + acc_y * Step/2.;
